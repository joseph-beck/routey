...
func (c Config) toSwaggerConfig() swaggerConfig {
	return swaggerConfig{
		...
		Oauth2DefaultClientID: c.Oauth2DefaultClientID,
	}
}
...
// CustomWrapHandler wraps `http.Handler` into `routey.HandlerFunc`.
func CustomWrapHandler(config *Config, handler *webdav.Handler) routey.HandlerFunc {
	var once sync.Once
	...
	return func(c *routey.Context) {
		...
		case "index.css":
			_ = css.Execute(c.Writer(), config.toSwaggerConfig())
		case "swagger-initializer.js":
			_ = js.Execute(c.Writer(), config.toSwaggerConfig())
		case "doc.json":
...
func DisablingWrapHandler(handler *webdav.Handler, envName string) routey.HandlerFunc {
	if os.Getenv(envName) != "" {
		return func(c *routey.Context) {
			c.Render(http.StatusNotFound, "")
		}
	}
...
// if specified environment variable passed.
func DisablingCustomWrapHandler(config *Config, handler *webdav.Handler, envName string) routey.HandlerFunc {
	if os.Getenv(envName) != "" {
		return func(c *routey.Context) {
			c.Render(http.StatusNotFound, "")
		}
